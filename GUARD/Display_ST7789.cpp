...
void LCD_Init(void)
{
  ...
  // MY=0 MX=1 MV=1 ML=0 BGR=0 MH=0 → 0x60 → landscape 320×240 (90° CW)
  LCD_WriteData(0x60);   
...
{
  // MADCTL=0x60 handles the 90° CW rotation in hardware.
  // CASET = column (X), RASET = row (Y) — direct mapping, no software swap needed.
